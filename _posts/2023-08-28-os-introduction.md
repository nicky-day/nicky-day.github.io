...
- 애플리케이션은 시스템 콜을 통해 커널에 접근할 수 있다.
...
- GUI는 그래픽 유저 인터페이스의 약자로 말 그대로 그래프로 된 인터페이스이다. 윈도우즈나 맥OS와 같이 그래픽으로 커널과 상호작용하기 때문에 일반 사용자도 사용하기 쉽다.
- CLI는 커맨드 라인 인터페이스의 약자로 유닉스나 리눅스 같은 운영체제가 기본적으로 제공하는 인터페이스로 텍스트를 이용해 커널과 상호작용한다. 



시스템 콜

- 커널은 사용자로부터 자신을 보호하기 위한 시스템 콜이라는 인터페이스를 가지고 있다. 사용자나 애플리케이션이 하드디스크에 데이터를 저장할 때, 시스템 콜 없이 하드디스크에 직접 접근하면 중요한 데이터를 덮어쓸 수도 있고, 해당 사용자나 애플리케이션이 저장한 데이터를 다른 애플리케이션이 덮어 쓸 수도 있다.
- 시스템 콜을 이용하면 커널에서 제공하는 라이트 함수를 쓰게 되는데, 그러면 운영체제가 알아서 하드디스크의 빈 공간에 저장하게 된다.



드라이버

- 하드웨어와 커널의 인터페이스로 드라이버를 사용한다.
- 운영체제는 많은 종류의 하드웨어를 전부 지원해야 하기 때문에 각각의 하드웨어에 맞는 프로그램을 커널이 미리 전부 갖고 있기 힘들다. 그래서 하드웨어를 제조한 제조사에서 드라이버를 만들어 제공하는 게 일반적이다.
- 보통 키보드나 마우스 같은 간단한 장치들은 커널에 포함되어 있어 컴퓨터에 꽂으면 바로 동작하지만, 그래픽 카드나 태블릿 같은 복잡한 장치들은 디바이스 드라이버를 설치해 사용해야 한다.
